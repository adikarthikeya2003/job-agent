...
COLUMNS = [
    "company", "title", "posted_date", "experience_required", "match_score",
    "role_family", "seniority", "location", "remote_status", "sponsorship_flag",
    "top_keywords", "apply_link", "source_tier",
]
...
def write_outputs(jobs: list, out_dir: Path, run_stamp: str):
    out_dir.mkdir(parents=True, exist_ok=True)
    ranked = sorted(jobs, key=lambda j: j.get("match_score", 0.0), reverse=True)
    rows = [_row(j) for j in ranked]

    csv_path = out_dir / f"jobs_{run_stamp}.csv"
    with csv_path.open("w", newline="") as f:
        writer = csv.DictWriter(f, fieldnames=COLUMNS)
        writer.writeheader()
        writer.writerows(rows)

    json_path = out_dir / f"jobs_{run_stamp}.json"
    json_path.write_text(json.dumps(rows, indent=2))

    md_path = out_dir / f"jobs_{run_stamp}.md"
...
(function(){
 ...
 [].forEach.call(tb.tHead.rows[0].cells,function(th,i){
  ...
  th.addEventListener('click',function(e){
   var at=-1;
   for(var k=0;k<levels.length;k++)if(levels[k].i===i)at=k;
   if(e.shiftKey){                      // shift-click adds a tie-breaker level
    if(at<0)levels.push({i:i,dir:th.dataset.dir==='asc'?1:-1});
    else levels[at].dir*=-1;
   }else{
    var dir=(at>=0&&levels.length===1)?levels[0].dir*-1
            :(th.dataset.dir==='asc'?1:-1);
    levels=[{i:i,dir:dir}];
   }
   apply();
  });
...
def write_html(jobs: list, out_dir: Path, run_stamp: str) -> dict:
    ...
    for j in ranked:
        ...
        rows.append(
            ...
            f'<td data-v="{co} {title}"><b>{co}</b><br><span class=t>{title}</span></td>'
    ...
    page = (
        ...
        + '<p class=hint>Click a column to sort · click again to reverse · '
          'shift-click a second column to break ties (Excel-style multi-level sort)</p>'
        + '<table><thead><tr>'
